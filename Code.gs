...
function getTestManager_(suiteName) {
  ...
  var expect = function(output, expected, evaluator) {
    ...
    testResult.description = suiteName + ' test #' + outcome.testResults.length + ': ' + (testResult.passed ? 'PASSED' : 'FAILED')
  ...
  var toString = function() {
    ...
    + function() { var ri, s=''; for (ri=0; ri<outcome.testResults.length; ri++) { s+=outcome.testResults[ri].description + '\n'; }; return s; }();
  };
...
function checkResult_(testManager, logger) {
  var defaultLogging = typeof logger !== 'function';
  var outcome = testManager.getOutcome();
  
  if (testManager.getOutcome().numFailed === 0) {
    defaultLogging ? Logger.log('%s - PASSED', testManager.testSuiteName()) : logger(testManager.getOutcome());
  }
  else {
    defaultLogging ? Logger.log('%s - FAILED\n%s', testManager.testSuiteName(), testManager.getOutcome().toString()) : logger(testManager.getOutcome());
  }
};
